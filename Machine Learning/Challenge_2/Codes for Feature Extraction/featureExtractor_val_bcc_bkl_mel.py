...
pathFolder = "/media/tushar/HDD/Academic/Semester_3/CAD_Computer Aided Diagnosis_5_Credits/Lab/Challenge-2 Data/val"#the pathfolder for Challenge-2 validation data.
folder_bcc = pathFolder + '/bcc/'
folder_bkl = pathFolder + '/bkl/'
folder_mel = pathFolder + '/mel/'

filesArrayBcc = [x for x in os.listdir(folder_bcc) if os.path.isfile(os.path.join(folder_bcc,x))]
filesArrayBkl = [x for x in os.listdir(folder_bkl) if os.path.isfile(os.path.join(folder_bkl,x))]
filesArrayMel = [x for x in os.listdir(folder_mel) if os.path.isfile(os.path.join(folder_mel,x))]
...
#Extracting the features and creating dataframe for all the bcc images.
for file_name in filesArrayBcc:
    file_name_no_extension = os.path.splitext(file_name)[0]
    image = cv.imread(folder_bcc+file_name)
...
#Extracting the features and creating dataframe for all the bkl images.
for file_name in filesArrayBkl:
    file_name_no_extension = os.path.splitext(file_name)[0]
    
    image = cv.imread(folder_bkl+file_name)
...
#Extracting the features and creating dataframe for all the mel images.
for file_name in filesArrayMel:
    file_name_no_extension = os.path.splitext(file_name)[0]
    
    image = cv.imread(folder_mel+file_name)
...
df.to_csv('val_fetures_extraction_bcc_bkl_mel.csv', index = False)
