...
pathFolder = "/media/tushar/HDD/Academic/Semester_3/CAD_Computer Aided Diagnosis_5_Credits/Lab/Testing Data/Challenge_2_testingData"#the pathfolder for Challenge-2 test data.
folder = pathFolder + '/test/'
filesArray_unsorted = [x for x in os.listdir(folder) if os.path.isfile(os.path.join(folder,x))]
...
#Extracting the features and creating dataframe for all the images.
for file_name in filesArray:
    file_name_no_extension = os.path.splitext(file_name)[0]
    image = cv.imread(folder+file_name)
...
df.to_csv('TEST_Challenge_2_fetures_extraction_bcc_bkl_mel.csv', index = False)
